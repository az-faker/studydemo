...
import com.demo.user.client.id.IdClient;
 ...
 */
@Slf4j
public class BaseDao<T extends BaseDO, P> {
	...
	private String getTableName(T t) {
		...
		if (ObjectUtil.isNotNull(tableAnnotation)) {
			return StrUtil.format("`{}`", tableAnnotation.value());
		} else {
	...
	private List<Field> getField(T t, Boolean ignoreNull) {
		...
		Stream<Field> fieldStream = CollUtil.toList(fields).stream().filter(field -> ObjectUtil.isNull(field.getAnnotation(Ignore.class)) || ObjectUtil.isNull(field.getAnnotation(Table.class).autoIncrease()));
